...
//          - Compute its measurement model jacobians (w.r.t robot state & w.r.t range and bearing )J_xr, J_z
//          - Compute cross variance of this new landmark with all other elements in state vector (before this landmark)
//              - 
...
void EkfSlam::update(){
    for (int i = 3; i < predictedState_x.rows; i += 2){ // Landmarks start at row 3
        ...
        float expectedBearing = atan2(yDiff, xDiff); // Expected landmark bearing based on the predicted pose
...
// TODO: Update the first 3 columns?
...
void EkfSlam::predictCovarianceMat(){
    ...
    Mat covarianceTopLeft = predictedCovariance_P(cv::Rect(0, 0, 3, 3));
...
bool EkfSlam::associateLandmark(int curreLandmarkIdx){
    return false;
}
